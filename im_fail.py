...
class GAPSO:
    ...
    def Filter(self):
        ...
        candidateindex = np.random.randint(0, self.popSize, 6)
        candidatelist = [self.population[i] for i in candidateindex]  # 0到50中选出6个

        candidatelist.sort(key=itemgetter(1))

        # 1里面存的是value  排出来的是升序  选最小的拿前面的
        return copy.deepcopy(candidatelist[0]), copy.deepcopy(candidatelist[1])
    ...
    def InitPop(self):
        ...
        for i in range(self.popSize):
            ...
            fit = self.CalFit(gene)
            self.population.append([gene, fit])
    ...
    def Crossover(self, father, mather):
        ...
        if np.random.rand() < self.crossoverRate:
            babby1[0][index:] = mather[0][index:]
            babby2[0][index:] = father[0][index:]

            babby1[1] = self.CalFit(babby1[0])
            babby2[1] = self.CalFit(babby2[0])
    ...
    def Mutation(self, people):
        ...
        if np.random.rand() < self.mutationRate:  # 0 to 1 random number
            ...
            people[0][index] = 1 - people[0][index]  # 1 变0  0 变成1
            people[1] = self.CalFit(people[0])
        return people
...
def Dubins_path_plan(UAV, target, R0):
    ...
    while abs(tagent_now_dis-tagent_target_dis)>deviation and tagent_now_dis < tagent_target_dis:
        new_point=np.array([start_site[0]+len_interval*np.cos(line_angle),start_site[1]+len_interval*np.sin(line_angle)])
...
if __name__ == '__main__':
    ...
    for t in np.arange(0, 140, time_interval):
        if t == 63.1:
            print('tets')
