...
import signal
'''
每架无一个状态位置0，表示正在搜索；1表示正在执行，攻击任务（此时路径已经规划好）；3 表示正在处理边界，
冲突消解，多人机由架无人机同时发现多个目标，给无人机和目标令牌，令牌多的无人机先选，选择令牌多的目标。关键问题，发现两个目标只能处理一个？
...
while sim_time<sim_num:
    ...
    for i in range(UAV_num):
        ...
        resource = np.random.randint(1, high=4, size=3)
    ...
    try:
        signal.alarm(400) # 开启闹钟信号设置时间2min
        ...
        filename = 'NSGA_data%s_%s_%s.mat' % (UAV_num, target_num,run_time)
        add_data=[UAV_num,target_num,1 - (np.sum(Targets_condition) / target_num)]
        if not os.path.exists(filename):
            io.savemat(filename,{'data':[add_data]})
        else:
            data=io.loadmat(filename)
            data['data']=np.row_stack((data['data'],add_data))
            io.savemat(filename, data)

        print('第%s次运行' % sim_time)
